/****************************************************************************
 * tests/unittest/framework_api/src/test_bt_l2cap.c
 *
...
int test_bt_l2cap_setup(FAR void **state)
{
    ...
    assert_non_null(ins); ins->app_id = 1; *state = ins; return 0;
}
...
void test_bt_l2cap_register_callbacks_normal(FAR void **state)
{
    ...
    l2cap_callbacks_t cbs; memset(&cbs, 0, sizeof(cbs));
...
{
    bt_instance_t *ins = *state;
    l2cap_config_option_t opt; memset(&opt, 0, sizeof(opt));
...
{
    bt_instance_t *ins = *state;
    bt_address_t addr; memset(&addr, 0x11, sizeof(addr));
    l2cap_config_option_t opt; memset(&opt, 0, sizeof(opt));
...
{
    bt_instance_t *ins = *state;
    bt_address_t addr; memset(&addr, 0x22, sizeof(addr));
