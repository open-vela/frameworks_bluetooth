/****************************************************************************
 * tests/unittest/framework_api/src/test_bt_hid_device.c
 *
...
static bt_status_t mock_send_report(bt_address_t *a, uint8_t id, uint8_t *d, int s) { check_expected_ptr(a); return mock_type(bt_status_t); }
static bt_status_t mock_response_report(bt_address_t *a, uint8_t t, uint8_t *d, int s) { check_expected_ptr(a); return mock_type(bt_status_t); }
static bt_status_t mock_report_error(bt_address_t *a, hid_status_error_t e) { check_expected_ptr(a); return mock_type(bt_status_t); }
static bt_status_t mock_virtual_unplug(bt_address_t *a) { check_expected_ptr(a); return mock_type(bt_status_t); }
...
void test_bt_hid_device_register_callbacks_normal(FAR void **state)
{
    ...
    hid_device_callbacks_t cbs; memset(&cbs, 0, sizeof(cbs));
...
{
    bt_instance_t *ins = *state;
    hid_device_sdp_settings_t sdp; memset(&sdp, 0, sizeof(sdp));
...
void test_bt_hid_device_send_report_normal(FAR void **state)
{
    ...
    expect_any(mock_send_report, a); will_return(mock_send_report, BT_STATUS_SUCCESS);
...
{
    bt_instance_t *ins = *state; ADDR_INIT(addr, 0x22);
    expect_any(mock_send_report, a); will_return(mock_send_report, BT_STATUS_FAIL);
...
void test_bt_hid_device_response_report_normal(FAR void **state)
{
    bt_instance_t *ins = *state; ADDR_INIT(addr, 0x11);
    uint8_t data[] = {0x01};
    expect_any(mock_response_report, a); will_return(mock_response_report, BT_STATUS_SUCCESS);
...
{
    bt_instance_t *ins = *state; ADDR_INIT(addr, 0x22);
    expect_any(mock_response_report, a); will_return(mock_response_report, BT_STATUS_FAIL);
...
{
    bt_instance_t *ins = *state; ADDR_INIT(addr, 0x11);
    expect_any(mock_report_error, a); will_return(mock_report_error, BT_STATUS_SUCCESS);
...
{
    bt_instance_t *ins = *state; ADDR_INIT(addr, 0x22);
    expect_any(mock_report_error, a); will_return(mock_report_error, BT_STATUS_FAIL);
...
{
    bt_instance_t *ins = *state; ADDR_INIT(addr, 0x11);
    expect_any(mock_virtual_unplug, a); will_return(mock_virtual_unplug, BT_STATUS_SUCCESS);
...
{
    bt_instance_t *ins = *state; ADDR_INIT(addr, 0x22);
    expect_any(mock_virtual_unplug, a); will_return(mock_virtual_unplug, BT_STATUS_FAIL);
