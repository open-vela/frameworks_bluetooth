...
static bt_scan_flush_ctrl_t* flush_ctrl_create(bt_scanner_t* scanner)
{
    ...
    bt_scan_flush_ctrl_t* ctrl = bt_calloc(1, sizeof(bt_scan_flush_ctrl_t));
    if (!ctrl) {
...
static void restart_flush_timer(bt_scanner_t* scanner)
{
    ...
    if (ctrl->flush_timer_active) {
        service_loop_cancel_timer(ctrl->flush_timer);
        ctrl->flush_timer = NULL;
        ctrl->flush_timer_active = false;
    }

    ctrl->flush_timer_active = true;
...
static void on_scan_result_cb(bt_scanner_t* scanner, ble_scan_result_t* result)
{
    ...
    if (result->length <= sizeof(cache->results[0].adv_data)) {
        if (!scan->flush_ctrl) {
    ...
    int fd, bt_instance_t* ins, bt_message_packet_t* packet)
{
    ...
    case BT_LE_SCAN_START: {
        ...
        scan->ins = ins;
        scan->scan_result_cache.count = 0;
        scan->flush_ctrl = NULL;
    ...
    int fd, bt_instance_t* ins, bt_message_packet_t* packet)
{
    ...
    case BT_LE_ON_SCAN_RESULT: {
        ...
        ble_scan_result_t* tmp = bt_malloc(sizeof(ble_scan_result_t) + result->length);
    ...
    case BT_LE_ON_BATCH_SCAN_RESULT: {
        ...
        for (int i = 0; i < packet->scan_batch_cb.count; ++i) {
            ...
            if (len == 0 || len > sizeof(packet->scan_batch_cb.results[i].adv_data))
                continue;

            ble_scan_result_t* tmp = bt_malloc(sizeof(ble_scan_result_t) + len);
            if (!tmp)
                continue;
