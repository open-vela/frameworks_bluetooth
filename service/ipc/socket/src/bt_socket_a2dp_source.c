...
const char* bt_a2dp_source_ipc_code_to_string(uint32_t code)
{
    switch (code) {
    CASE_RETURN_STR(BT_A2DP_SOURCE_MESSAGE_START);
    CASE_RETURN_STR(BT_A2DP_SOURCE_REGISTER_CALLBACKS);
    CASE_RETURN_STR(BT_A2DP_SOURCE_UNREGISTER_CALLBACKS);
    CASE_RETURN_STR(BT_A2DP_SOURCE_IS_CONNECTED);
    CASE_RETURN_STR(BT_A2DP_SOURCE_IS_PLAYING);
    CASE_RETURN_STR(BT_A2DP_SOURCE_GET_CONNECTION_STATE);
    CASE_RETURN_STR(BT_A2DP_SOURCE_CONNECT);
    CASE_RETURN_STR(BT_A2DP_SOURCE_DISCONNECT);
    CASE_RETURN_STR(BT_A2DP_SOURCE_SET_SILENCE_DEVICE);
    CASE_RETURN_STR(BT_A2DP_SOURCE_SET_ACTIVE_DEVICE);
    CASE_RETURN_STR(BT_A2DP_SOURCE_MESSAGE_END);
    CASE_RETURN_STR(BT_A2DP_SOURCE_CALLBACK_START);
    CASE_RETURN_STR(BT_A2DP_SOURCE_CONNECTION_STATE_CHANGE);
    CASE_RETURN_STR(BT_A2DP_SOURCE_AUDIO_STATE_CHANGE);
    CASE_RETURN_STR(BT_A2DP_SOURCE_CONFIG_CHANGE);
    CASE_RETURN_STR(BT_A2DP_SOURCE_CALLBACK_END);
    CASE_RETURN_STR(BT_IPC_CODE_COMMAND_A2DP_SRC_BEGIN);
    CASE_RETURN_STR(BT_IPC_CODE_COMMAND_A2DP_SRC_END);
    CASE_RETURN_STR(BT_IPC_CODE_CALLBACK_A2DP_SRC_BEGIN);
    CASE_RETURN_STR(BT_IPC_CODE_CALLBACK_A2DP_SRC_END);
    default:
        return NULL;
    }
}
