...
static int bt_socket_server_receive(service_poll_t* poll, int fd, void* userdata)
{
    ...
    } else if (BT_IPC_CODE_CHECK_RANGE(packet->code, BT_IPC_CODE_COMMAND_AURACAST_SINK_BEGIN, BT_IPC_CODE_COMMAND_AURACAST_SINK_END)) {
        bt_socket_server_auracast_sink_process(poll, fd, ins, packet);
#endif
