 ...
 ***************************************************************************/
#ifndef __LEA_CCPC_EVENT_H__
#define __LEA_CCPC_EVENT_H__
...
typedef struct {
    ...
    uint8_t valueint8_2;
    uint8_t dataarry[1];
} lea_ccpc_event_data_t;

typedef struct {
    bt_address_t remote_addr;
    lea_ccpc_event_t event;
    lea_ccpc_event_data_t event_data;
} lea_ccpc_msg_t;

/****************************************************************************
 * Public Functions
 ****************************************************************************/
lea_ccpc_msg_t *lea_ccpc_msg_new(lea_ccpc_event_t event, bt_address_t *remote_addr, uint32_t tbs_id);
lea_ccpc_msg_t *lea_ccpc_msg_new_ext(lea_ccpc_event_t event, bt_address_t *remote_addr, uint32_t tbs_id, size_t size);
void lea_ccpc_msg_destory(lea_ccpc_msg_t *ccp_msg);

#endif /* __LEA_CCPC_EVENT_H__ */