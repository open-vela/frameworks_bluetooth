 ...
 ***************************************************************************/
#ifndef __BT_AG_TAPI_SERVICE_H__
#define __BT_AG_TAPI_SERVICE_H__
...
#define TELE_MAX_PHONE_NUMBER_LENGTH 100
#define TELE_MAX_INCOMING_LINE_LENGTH 100
#define TELE_MAX_CALLER_NAME_LENGTH 100
#define TELE_MAX_START_TIME_LENGTH 100
#define TELE_MAX_OPERATOR_NAME_LENGTH 63
#define TELE_MAX_CALL_ID_LENGTH 100
typedef struct tele_call_ {
    uint16_t index;
    char call_id[TELE_MAX_CALL_ID_LENGTH + 1];
    uint8_t call_state;
    char line_identification[TELE_MAX_PHONE_NUMBER_LENGTH + 1];
    bool is_multiparty;
    bool is_incoming;
} tele_call_info_t;

typedef struct tell_network_ {
    uint8_t reg_state;
    char operator_name[TELE_MAX_OPERATOR_NAME_LENGTH + 1];
    uint8_t roaming_type;
} tele_registration_info;
...
bt_status_t tele_service_get_network_info(hfp_network_state_t* network,
    ...
    uint8_t* signal);
#endif /* __HFP_AG_TAPI_SERVICE_H__ */
