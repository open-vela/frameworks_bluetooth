 ...
 * Included Files
 ****************************************************************************/
#define LOG_TAG "lea_mcps_service"
...
#define CHECK_ENABLED()                   \
    {                                     \
        if (!g_mcps_service.started)       \
            return BT_STATUS_NOT_ENABLED; \
    }

#define MCPS_CALLBACK_FOREACH(_list, _cback, ...) BT_CALLBACK_FOREACH(_list, lea_mcps_callbacks_t, _cback, ##__VA_ARGS__)
...
static mcps_service_t g_mcps_service = {
    .started = false,
    .callbacks = NULL,
};
...
static void *lea_mcs_set_callbacks(void *handle, lea_mcps_callbacks_t* callbacks);
static bool lea_mcs_reset_callbacks(void **handle, void *cookie);

static void lea_mcps_process_message(void *data)
{
    mcps_event_t *msg = (mcps_event_t *)data;
    BT_LOGD("%s, msg->event:%d ", __func__, msg->event);
    switch (msg->event) {
        case MCS_STATE: {
            MCPS_CALLBACK_FOREACH(g_mcps_service.callbacks, mcs_state_cb, msg->event);
            break;
        }
        case MCS_PLAYER_SET: {
    ...
    mcps_event_destory(msg);
}

static bt_status_t lea_mcps_send_msg(mcps_event_t *msg)
{
    assert(msg);

    do_in_service_loop(lea_mcps_process_message, msg);
 ...
 * sal callbacks
 ****************************************************************************/
void lea_on_mcps_state(uint32_t mcs_id, uint8_t ccid, bool added)
{
    BT_LOGD("%s ", __func__);
    mcps_event_t *event;

    event = mcps_event_new(MCS_STATE, mcs_id);
    if (!event) {
    ...
    lea_mcps_send_msg(event);
}

void lea_on_mcps_player_info_set_result(uint32_t mcs_id, void *player_ref, bool result)
{
    BT_LOGD("%s ", __func__);
    mcps_event_t *event;

    event = mcps_event_new(MCS_PLAYER_SET, mcs_id);
    if (!event) {
        BT_LOGE("%s, Failed to create msg", __func__);
        return;
    }
...
void lea_on_mcps_object_added_result(uint32_t mcs_id, void *obj_ref, lea_object_id obj_id)
{
...
void lea_on_mcps_set_position_result(uint32_t mcs_id, int32_t position)
{
    BT_LOGD("%s, position:%d ", __func__, position);
    mcps_event_t *event;

    event = mcps_event_new(MCS_SET_POSITION, mcs_id);
    if (!event) {
        BT_LOGE("%s, Failed to create msg", __func__);
        return;
    }
...
void lea_on_mcps_set_playback_speed_result(uint32_t mcs_id, int8_t speed)
{
    BT_LOGD("%s, speed:%d ", __func__, speed);
    mcps_event_t *event;

    event = mcps_event_new(MCS_SET_PLAYBACK_SPEED, mcs_id);
    if (!event) {
        BT_LOGE("%s, Failed to create msg", __func__);
        return;
    }
...
void lea_on_mcps_set_current_track_result(uint32_t mcs_id, lea_object_id track_id)
{
    BT_LOGD("%s ", __func__);
    mcps_event_t *event;

    event = mcps_event_new(MCS_SET_CURRENT_TRACK, mcs_id);
    if (!event) {
        BT_LOGE("%s, Failed to create msg", __func__);
        return;
    }
...
void lea_on_mcps_set_next_track_result(uint32_t mcs_id, lea_object_id track_id)
{
    BT_LOGD("%s", __func__);
    mcps_event_t *event;

    event = mcps_event_new(MCS_SET_NEXT_TRACK, mcs_id);
    if (!event) {
        BT_LOGE("%s, Failed to create msg", __func__);
        return;
    }
...
void lea_on_mcps_set_current_group_result(uint32_t mcs_id, lea_object_id group_id)
{
    BT_LOGD("%s ", __func__);
    mcps_event_t *event;

    event = mcps_event_new(MCS_SET_CURRENT_GROUP, mcs_id);
    if (!event) {
        BT_LOGE("%s, Failed to create msg", __func__);
        return;
    }
...
void lea_on_mcps_set_playing_order_result(uint32_t mcs_id, uint8_t order)
{
    BT_LOGD("%s ", __func__);
    mcps_event_t *event;

    event = mcps_event_new(MCS_SET_PLAYING_ORDER, mcs_id);
    if (!event) {
        BT_LOGE("%s, Failed to create msg", __func__);
        return;
    }
...
void lea_on_mcps_play_result(uint32_t mcs_id)
{
    BT_LOGD("%s ", __func__);
    mcps_event_t *event;

    event = mcps_event_new(MCS_CONTROL_POINT_PLAY, mcs_id);
    if (!event) {
        BT_LOGE("%s, Failed to create msg", __func__);
        return;
    }
...
void lea_on_mcps_pause_result(uint32_t mcs_id)
{
    BT_LOGD("%s ", __func__);
    mcps_event_t *event;

    event = mcps_event_new(MCS_CONTROL_POINT_PAUSE, mcs_id);
    if (!event) {
        BT_LOGE("%s, Failed to create msg", __func__);
        return;
    }
...
void lea_on_mcps_fast_rewind_result(uint32_t mcs_id)
{
    BT_LOGD("%s ", __func__);
    mcps_event_t *event;

    event = mcps_event_new(MCS_CONTROL_POINT_FAST_REWIND, mcs_id);
    if (!event) {
        BT_LOGE("%s, Failed to create msg", __func__);
        return;
    }
...
void lea_on_mcps_fast_forward_result(uint32_t mcs_id)
{
    BT_LOGD("%s ", __func__);
    mcps_event_t *event;

    event = mcps_event_new(MCS_CONTROL_POINT_FAST_FORWARD, mcs_id);
    if (!event) {
        BT_LOGE("%s, Failed to create msg", __func__);
        return;
    }
...
void lea_on_mcps_stop_result(uint32_t mcs_id)
{
    BT_LOGD("%s ", __func__);
    mcps_event_t *event;

    event = mcps_event_new(MCS_CONTROL_POINT_STOP, mcs_id);
    if (!event) {
        BT_LOGE("%s, Failed to create msg", __func__);
        return;
    }
...
void lea_on_mcps_move_result(uint32_t mcs_id, int32_t offset)
{
    BT_LOGD("%s ", __func__);
    mcps_event_t *event;

    event = mcps_event_new(MCS_CONTROL_POINT_MOVE, mcs_id);
    if (!event) {
        BT_LOGE("%s, Failed to create msg", __func__);
        return;
    }
...
void lea_on_mcps_previous_segment_result(uint32_t mcs_id)
{
    BT_LOGD("%s ", __func__);
    mcps_event_t *event;

    event = mcps_event_new(MCS_CONTROL_POINT_PREVIOUS_SEGMENT, mcs_id);
    if (!event) {
        BT_LOGE("%s, Failed to create msg", __func__);
        return;
    }
...
void lea_on_mcps_next_segment_result(uint32_t mcs_id)
{
    BT_LOGD("%s ", __func__);
    mcps_event_t *event;

    event = mcps_event_new(MCS_CONTROL_POINT_NEXT_SEGMENT, mcs_id);
    if (!event) {
        BT_LOGE("%s, Failed to create msg", __func__);
        return;
    }
...
void lea_on_mcps_first_segment_result(uint32_t mcs_id)
{
    BT_LOGD("%s ", __func__);
    mcps_event_t *event;

    event = mcps_event_new(MCS_CONTROL_POINT_FIRST_SEGMENT, mcs_id);
    if (!event) {
        BT_LOGE("%s, Failed to create msg", __func__);
        return;
    }
...
void lea_on_mcps_last_segment_result(uint32_t mcs_id)
{
    BT_LOGD("%s ", __func__);
    mcps_event_t *event;

    event = mcps_event_new(MCS_CONTROL_POINT_LAST_SEGMENT, mcs_id);
    if (!event) {
        BT_LOGE("%s, Failed to create msg", __func__);
        return;
    }
...
void lea_on_mcps_goto_segment_result(uint32_t mcs_id, int32_t n_segment)
{
    BT_LOGD("%s, segment num:%d ", __func__, n_segment);
    mcps_event_t *event;

    event = mcps_event_new(MCS_CONTROL_POINT_GOTO_SEGMENT, mcs_id);
    if (!event) {
        BT_LOGE("%s, Failed to create msg", __func__);
        return;
    }
...
void lea_on_mcps_previous_track_result(uint32_t mcs_id)
{
    BT_LOGD("%s ", __func__);
    mcps_event_t *event;

    event = mcps_event_new(MCS_CONTROL_POINT_PREVIOUS_TRACK, mcs_id);
    if (!event) {
        BT_LOGE("%s, Failed to create msg", __func__);
        return;
    }
...
void lea_on_mcps_next_track_result(uint32_t mcs_id)
{
    BT_LOGD("%s ", __func__);
    mcps_event_t *event;

    event = mcps_event_new(MCS_CONTROL_POINT_NEXT_TRACK, mcs_id);
    if (!event) {
        BT_LOGE("%s, Failed to create msg", __func__);
        return;
    }
...
void lea_on_mcps_first_track_result(uint32_t mcs_id)
{
    BT_LOGD("%s ", __func__);
    mcps_event_t *event;

    event = mcps_event_new(MCS_CONTROL_POINT_FIRST_TRACK, mcs_id);
    if (!event) {
        BT_LOGE("%s, Failed to create msg", __func__);
        return;
    }
...
void lea_on_mcps_last_track_result(uint32_t mcs_id)
{
    BT_LOGD("%s ", __func__);
    mcps_event_t *event;

    event = mcps_event_new(MCS_CONTROL_POINT_LAST_TRACK, mcs_id);
    if (!event) {
        BT_LOGE("%s, Failed to create msg", __func__);
        return;
    }
...
void lea_on_mcps_goto_track_result(uint32_t mcs_id, int32_t n_track)
{
    BT_LOGD("%s, track num:%d", __func__, n_track);
    mcps_event_t *event;

    event = mcps_event_new(MCS_CONTROL_POINT_GOTO_TRACK, mcs_id);
    if (!event) {
        BT_LOGE("%s, Failed to create msg", __func__);
        return;
    }
...
void lea_on_mcps_previous_group_result(uint32_t mcs_id)
{
    BT_LOGD("%s ", __func__);
    mcps_event_t *event;

    event = mcps_event_new(MCS_CONTROL_POINT_PREVIOUS_GROUP, mcs_id);
    if (!event) {
        BT_LOGE("%s, Failed to create msg", __func__);
        return;
    }
...
void lea_on_mcps_next_group_result(uint32_t mcs_id)
{
    BT_LOGD("%s ", __func__);
    mcps_event_t *event;

    event = mcps_event_new(MCS_CONTROL_POINT_NEXT_GROUP, mcs_id);
    if (!event) {
        BT_LOGE("%s, Failed to create msg", __func__);
        return;
    }
...
void lea_on_mcps_first_group_result(uint32_t mcs_id)
{
    BT_LOGD("%s ", __func__);
    mcps_event_t *event;

    event = mcps_event_new(MCS_CONTROL_POINT_FIRST_GROUP, mcs_id);
    if (!event) {
        BT_LOGE("%s, Failed to create msg", __func__);
        return;
    }
...
void lea_on_mcps_last_group_result(uint32_t mcs_id)
{
    BT_LOGD("%s ", __func__);
    mcps_event_t *event;

    event = mcps_event_new(MCS_CONTROL_POINT_LAST_GROUP, mcs_id);
    if (!event) {
        BT_LOGE("%s, Failed to create msg", __func__);
        return;
    }
...
void lea_on_mcps_goto_group_result(uint32_t mcs_id, int32_t n_group)
{
    BT_LOGD("%s, group num:%d ", __func__, n_group);
    mcps_event_t *event;

    event = mcps_event_new(MCS_CONTROL_POINT_GOTO_GROUP, mcs_id);
    if (!event) {
        BT_LOGE("%s, Failed to create msg", __func__);
        return;
    }
...
void lea_on_mcps_search_track_name_result(uint32_t mcs_id, size_t size, char *name, bool last_condition)
{
    BT_LOGD("%s, last_condition:%d ", __func__, last_condition);
    mcps_event_t *event;

    event = mcps_event_new_ext(MCS_SEARCH_TRACK_NAME, mcs_id, size);
    if (!event) {
        BT_LOGE("%s, Failed to create msg", __func__);
        return;
    }
...
void lea_on_mcps_search_artist_name_result(uint32_t mcs_id, size_t size, char *name, bool last_condition)
{
    BT_LOGD("%s, last_condition:%d ", __func__, last_condition);
    mcps_event_t *event;

    event = mcps_event_new_ext(MCS_SEARCH_ARTIST_NAME, mcs_id, size);
    if (!event) {
        BT_LOGE("%s, Failed to create msg", __func__);
        return;
    }
...
void lea_on_mcps_search_album_name_result(uint32_t mcs_id, size_t size, char *name, bool last_condition)
{
    BT_LOGD("%s, last_condition:%d ", __func__, last_condition);
    mcps_event_t *event;

    event = mcps_event_new_ext(MCS_SEARCH_ALBUM_NAME, mcs_id, size);
    if (!event) {
        BT_LOGE("%s, Failed to create msg", __func__);
        return;
    }
...
void lea_on_mcps_search_group_name_result(uint32_t mcs_id, size_t size, char *name, bool last_condition)
{
    BT_LOGD("%s, last_condition:%d ", __func__, last_condition);
    mcps_event_t *event;

    event = mcps_event_new_ext(MCS_SEARCH_GROUP_NAME, mcs_id, size);
    if (!event) {
        BT_LOGE("%s, Failed to create msg", __func__);
        return;
    }
...
void lea_on_mcps_search_earliest_year_result(uint32_t mcs_id, size_t size, char *year, bool last_condition)
{
    BT_LOGD("%s, last_condition:%d ", __func__, last_condition);
    mcps_event_t *event;

    event = mcps_event_new_ext(MCS_SEARCH_EARLIEST_YEAR, mcs_id, size);
    if (!event) {
        BT_LOGE("%s, Failed to create msg", __func__);
        return;
    }
...
void lea_on_mcps_search_latest_year_result(uint32_t mcs_id, size_t size, char *year, bool last_condition)
{
    BT_LOGD("%s, last_condition:%d ", __func__, last_condition);
    mcps_event_t *event;

    event = mcps_event_new_ext(MCS_SEARCH_LATEST_YEAR, mcs_id, size);
    if (!event) {
        BT_LOGE("%s, Failed to create msg", __func__);
        return;
    }
...
void lea_on_mcps_search_genre_result(uint32_t mcs_id, size_t size, char *name, bool last_condition)
{
    BT_LOGD("%s, last_condition:%d ", __func__, last_condition);
    mcps_event_t *event;

    event = mcps_event_new_ext(MCS_SEARCH_GENRE, mcs_id, size);
    if (!event) {
        BT_LOGE("%s, Failed to create msg", __func__);
        return;
    }
...
void lea_on_mcps_search_tracks_result(uint32_t mcs_id, bool last_condition)
{
    BT_LOGD("%s, last_condition:%d ", __func__, last_condition);
    mcps_event_t *event;

    event = mcps_event_new(MCS_SEARCH_TRACKS, mcs_id);
...
void lea_on_mcps_search_groups_result(uint32_t mcs_id, bool last_condition)
{
    BT_LOGD("%s, last_condition:%d ", __func__, last_condition);
    mcps_event_t *event;

    event = mcps_event_new(MCS_SEARCH_GROUPS, mcs_id);
    if (!event) {
        BT_LOGE("%s, Failed to create msg", __func__);
        return;
    }
...
static bt_status_t lea_mcs_add()
{
    ...
    pthread_mutex_lock(&g_mcps_service.device_lock);
    ret = bt_sal_lea_mcs_add(lea_mcs_id);
    if (ret != BT_STATUS_SUCCESS) {
        pthread_mutex_unlock(&g_mcps_service.device_lock);
        return BT_STATUS_FAIL;
    }
    pthread_mutex_unlock(&g_mcps_service.device_lock);
...
static void *lea_mcs_set_callbacks(void *handle, lea_mcps_callbacks_t* callbacks)
{
    ...
    return bt_remote_callbacks_register(g_mcps_service.callbacks, handle, (void *)callbacks);
}
...
static bool lea_mcs_reset_callbacks(void **handle, void *cookie)
{
    ...
    return bt_remote_callbacks_unregister(g_mcps_service.callbacks, handle, cookie);
}


static const lea_mcps_interface_t leMcsInterface = {
 ...
 * Public function
 ****************************************************************************/
static const void *get_lea_mcps_profile_interface(void)
{
    return &leMcsInterface;
}

static bt_status_t lea_mcps_init(void)
{
    BT_LOGD("%s", __func__);
    return BT_STATUS_SUCCESS;
}

static bt_status_t lea_mcps_startup(profile_on_startup_t cb)
{
    BT_LOGD("%s", __func__);
    bt_status_t status;
    pthread_mutexattr_t attr;
    mcps_service_t *service = &g_mcps_service;
    if (service->started)
        return BT_STATUS_SUCCESS;
...
static bt_status_t lea_mcps_shutdown(profile_on_shutdown_t cb)
{
    if (!g_mcps_service.started)
        return BT_STATUS_SUCCESS;
    ...
    g_mcps_service.started = false;

    bt_callbacks_list_free(g_mcps_service.callbacks);
    g_mcps_service.callbacks = NULL;
    pthread_mutex_unlock(&g_mcps_service.device_lock);
    pthread_mutex_destroy(&g_mcps_service.device_lock);
    return BT_STATUS_SUCCESS;
}

static void lea_mcps_cleanup(void)
{
    BT_LOGD("%s", __func__);
}

static int lea_mcps_dump(void)
{
    printf("impl leaudio mcps dump");
    return 0;
}

static const profile_service_t lea_mcps_service = {
    .auto_start = true,
    .name = PROFILE_MCPS_NAME,
    .id = PROFILE_LEAUDIO_MCPS,
    .transport = BT_TRANSPORT_BLE,
    .uuid = {BT_UUID128_TYPE, { 0 }},
    .init = lea_mcps_init,
    .startup = lea_mcps_startup,
    .shutdown = lea_mcps_shutdown,
    .process_msg = NULL,
    .get_state = NULL,
    .get_profile_interface = get_lea_mcps_profile_interface,
    .cleanup = lea_mcps_cleanup,
    .dump = lea_mcps_dump,
};

void register_lea_mcps_service(void)
{
    register_service(&lea_mcps_service);
}
