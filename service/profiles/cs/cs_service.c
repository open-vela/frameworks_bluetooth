...
static void cs_startup(profile_on_startup_t startup);
static void cs_shutdown(profile_on_shutdown_t shutdown);
...
static void cs_service_handle_event(void* data)
{
    ...
    switch (msg->id) {
    case CS_STARTUP:
        cs_startup((profile_on_startup_t)msg->data.cb);
        break;
    case CS_SHUTDOWN:
        cs_shutdown((profile_on_shutdown_t)msg->data.cb);
        break;
    default: {
...
static void do_in_cs_service(avrcp_msg_t* msg)
{
    if (msg == NULL)
        return;
...
static void cs_startup(profile_on_startup_t startup)
{
    startup(PROFILE_CS, true);
}

static void cs_shutdown(profile_on_shutdown_t shutdown)
{
    shutdown(PROFILE_CS, true);
}
...
static bt_status_t cs_shutdown(profile_on_shutdown_t cb)
{
    cs_msg_t* msg = avrcp_msg_new(CS_SHUTDOWN, NULL);
    msg->data.cb = cb;
    do_in_cs_service(msg);
...
{
    return bt_cs_unregister(g_cs_service.callbacks, remote, cookie);
}
...
static bt_status_t cs_start_distance_measurement( bt_distance_measurement_params_t* params)
{
    ...
    case METHOD_RSSI:
        ...
        break;
    case METHOD_CS:{
        cs_msg_t* msg = avrcp_msg_new(START_REQ, &params->addr);
