...
typedef struct cs_state_machine {
    state_machine_t sm;
    void* service;
    bt_address_t addr;
...
static bool wait_for_config_complete_event(state_machine_t* sm, uint32_t event, void* p_data);
...
static const state_t wait_for_security_complete_state = {
    ...
    .process_event = wait_for_security_process_event,
};
...
{
    cs_state_machine_t* cs_sm = (cs_state_machine_t*)sm;
    cs_event_data_t* data = (cs_event_data_t*)p_data;

    CS_DBG_EVENT(sm, &cs_sm->addr, event);
    switch (event) {
    case CONNECTED_EVT:
        hsm_transition(sm, &connected_state);
        break;
    default:
        break;
    }
...
static bool init_process_event(state_machine_t* sm, uint32_t event, void* p_data)
{
    ...
    case DISCONNECTED_EVT:
        hsm_transition(sm, &stopped_state);
        break;
    default:
        break;
    }
...
static bool connected_process_event(state_machine_t* sm, uint32_t event, void* p_data)
{
    ...
    case START_REQ:
        ...
        if (cs_sm->is_capbilities_exchanged) {
            // bt_sal_cs_set_default_settings(PRIMARY_ADAPTER, &data->bd_addr, params->role, params->antenna_paths_mask);
...
static bool wait_for_config_complete_process_event(state_machine_t* sm, uint32_t event, void* p_data)
{
    ...
    case CONFIG_DONE_EVT:
        hsm_transition(sm, &wait_for_security_complete_state);
        break;
    default:
        break;
    }
...
static bool wait_for_security_complete_process_event(state_machine_t* sm, uint32_t event, void* p_data)
{
    ...
    case SECURITY_DONE_EVT:
        hsm_transition(sm, &wait_for_procedure_complete_state);
        break;
    default:
        break;
    }
...
static bool wait_for_procedure_complete_process_event(state_machine_t* sm, uint32_t event, void* p_data)
{
    ...
    case PROCEDURE_DONE_EVT:
        hsm_transition(sm, &started_state);
        break;
    default:
        break;
    }
...
void cs_state_machine_handle_event(cs_state_machine_t* sm,
    cs_msg_t* msg)
{
