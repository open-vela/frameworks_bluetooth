...
static struct net_buf* get_rx(const uint8_t* buf)
{
    ...
    case BT_HCI_H4_ISO:
        ...
        break;
    default:
        BT_LOGE("Unknown RX packet type 0x%02x", buf[0]);
        break;
    }
...
static int h4_open(const struct device* dev, bt_hci_recv_t recv, void* hci_data)
{
    ...
    if (fd < 0) {
        syslog(1, "H4: Failed to open %s: %d", CONFIG_BT_UART_ON_DEV_NAME, errno);
        return fd;
    }
    ...
    bt_dev = dev;
    syslog(1, "H4: %s opened as fd:%d", CONFIG_BT_UART_ON_DEV_NAME, h4->fd);
...
static int h4_send(const struct device* dev, struct net_buf* buf)
{
    ...
    default:
        BT_LOGE("H4: Unknown buffer type");
        net_buf_unref(buf);
        return -EINVAL;
    }
    ...
    if (ret != len) {
        BT_LOGE("H4: Failed to send %d bytes: %d", len, ret);
        ret = -EINVAL;
    }
