...
static void hid_connect_callback(struct bt_hid_device* hid)
{
    ...
    bt_sal_cm_profile_connected_callback(cm_data_new(&hid_conn->addr, PROFILE_HID_DEV));
    bt_sal_profile_disconnect_register(&hid_conn->addr, PROFILE_HID_DEV, PRIMARY_ADAPTER, hid_disconnect_handler, hid_conn);
}
...
bt_status_t bt_sal_hid_device_connect(bt_address_t* addr)
{
    ...
    status = bt_sal_profile_connect_request(addr, PROFILE_HID_DEV, PRIMARY_ADAPTER, hid_connect_handler, NULL);
...
static bt_status_t hid_disconnect_handler(bt_controller_id_t id, bt_address_t* bd_addr, void* user_data)
{
    sal_hid_connection_t* hid_conn = (sal_hid_connection_t*)user_data;
    int ret;
...
bt_status_t bt_sal_hid_device_disconnect(bt_address_t* addr)
{
    ...
    return bt_sal_profile_disconnect_request(&hid_conn->addr, PROFILE_HID_DEV, PRIMARY_ADAPTER, hid_disconnect_handler, hid_conn);
}
