...
static lea_mcps_interface_t *get_profile_service(void)
{
    return (lea_mcps_interface_t *)service_manager_get_profile(PROFILE_LEAUDIO_MCPS);
}

void *bt_lea_mcps_register_callbacks(bt_instance_t *ins, const lea_mcps_callbacks_t *callbacks)
{
    lea_mcps_interface_t *profile = get_profile_service();

    return profile->set_callbacks(NULL, (lea_mcps_callbacks_t *)callbacks);
}
