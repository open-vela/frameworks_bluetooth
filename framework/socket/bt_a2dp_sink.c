...
bt_status_t bt_a2dp_sink_set_bitpool(bt_instance_t* ins, uint8_t bitpool)
{
    bt_message_packet_t packet;
    bt_status_t status;

    BT_SOCKET_INS_VALID(ins, BT_STATUS_PARM_INVALID);
    packet.a2dp_sink_pl._bt_a2dp_sink_set_bitpool.bitpool = bitpool;

    status = bt_socket_client_sendrecv(ins, &packet, BT_A2DP_SINK_SET_BITPOOL);
    if (status != BT_STATUS_SUCCESS) {
        return status;
    }
