...
{
    BpBleGattServer *gatts = (BpBleGattServer *)bluetooth_get_proxy(ins, PROFILE_GATTS);

    IBleGattServerCallbacks *cbks = BleGattServerCallbacks_new(callbacks);
    AIBinder *binder = BleGattServerCallbacks_getBinder(cbks);
    if (!binder) {
        BleGattServerCallbacks_delete(cbks);
        return BT_STATUS_FAIL;
    }

    void *handle = BpBleGattServer_registerService(gatts, binder);
    if (!handle) {
        BleGattServerCallbacks_delete(cbks);
        return BT_STATUS_FAIL;
    }
...
bt_status_t ble_gatts_unregister_service(gatts_handle_t srv_handle)
{
    ...
    bt_status_t status = BpBleGattServer_unregisterService(cbks->proxy, cbks->cookie);
...
{
    IBleGattServerCallbacks *cbks = srv_handle;
    return BpBleGattServer_connect(cbks->proxy, cbks->cookie, addr, addr_type);
}

bt_status_t ble_gatts_disconnect(gatts_handle_t srv_handle)
{
    IBleGattServerCallbacks *cbks = srv_handle;
    return BpBleGattServer_disconnect(cbks->proxy, cbks->cookie);
}

bt_status_t ble_gatts_create_service_table(gatts_handle_t srv_handle, gatt_srv_db_t *srv_db)
{
    IBleGattServerCallbacks *cbks = srv_handle;
    bt_status_t status = BpBleGattServer_createServiceTable(cbks->proxy, cbks->cookie, srv_db);
    if (status == BT_STATUS_SUCCESS)
        cbks->srv_db = srv_db;
    return status;
}
...
{
    IBleGattServerCallbacks *cbks = srv_handle;
    return BpBleGattServer_start(cbks->proxy, cbks->cookie);
}

bt_status_t ble_gatts_stop(gatts_handle_t srv_handle)
{
    IBleGattServerCallbacks *cbks = srv_handle;
    return BpBleGattServer_stop(cbks->proxy, cbks->cookie);
}

bt_status_t ble_gatts_response(gatts_handle_t srv_handle, uint32_t req_handle, uint8_t *value, uint16_t length)
{
    IBleGattServerCallbacks *cbks = srv_handle;
    return BpBleGattServer_response(cbks->proxy, cbks->cookie, req_handle, value, length);
}
