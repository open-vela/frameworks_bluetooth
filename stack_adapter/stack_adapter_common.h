...
typedef enum {
    SERVICE_EVENT_DONE = 0,
    SNIFF_START_MONITOR = 1,   /* 副耳启动监听上报 */
    SNIFF_START_MINITOR_REQ,   /* 启动监听 */
    SNIFF_PAUSE_MONITOR_REQ,   /* 暂停监听 */
    SNIFF_START_MONITOR_CMP,   /* 监听启动完成消息 */
    SNIFF_PAUSE_MONITOR_CMP,   /* 监听暂停完成消息 */
    SNIFF_STOP_MONITOR_REQ,    /* 停止监听请求 */
    SNIFF_STOP_MONITOR_CMP,    /* 监听停止完成消息 */
    SNIFF_PAUSE_MONITOR = 8,   /* 副耳暂停监听上报 */
    SNIFF_START_HANDOVER_REQ,  /* 主副耳service消息 */
    SNIFF_START_HANDOVER_CMP,  /* 主副切换完成 */
    SNIFF_START_HANDOVER,      /* 副耳主副切换上报 */
    SNIFF_CHANGE_ROL_IND,      /* 主副耳角色切换上报 */
    SNIFF_HANOVER_PREPARE_CMP, /* 主副切换准备切完成 */
    SNIFF_TWS_SPP_CONNECTED,
    SNIFF_TWS_SPP_DISCONNECTED,
    SNIFF_TWS_RECEIVED_SERVICE_DATA,
    SNIFF_SERVICE_MSG_MAX
} SERVICE_SNIFF_MSG;

typedef enum {
    SNIFF_STATE_IDLE = 0x00,
    SNIFF_STATE_OPENING,
    SNIFF_STATE_READY,
    SNIFF_STATE_HANDOVER,
    SNIFF_STATE_SUSPENDING
} SNIFF_STATUS_E;

typedef enum {
    SNIFF_HANDOVER_SINGLE_NO_CONNECT,
    SNIFF_HANDOVER_DUAL_NO_CONNECT,
    SNIFF_HANDOVER_DUAL_NO_MONITOR,
    SNIFF_HANDOVER_DUAL_IN_MONITOR
} SNIFF_HANDOVER_SUBSTATUS_E;

typedef enum { SNIFF_A2DP_STREAMING, SNIFF_A2DP_NONE = 0xFF } SNIFF_A2DP_STATE_E;

typedef enum { SNIFF_HFP_ACTIVE_CALL, SNIFF_HFP_NO_CALL = 0xFF } SNIFF_HFP_STATE_E;

typedef SERVICE_SNIFF_MSG SERVICE_SNIFF_MSG_E;

/* sniff_start/sniff_pause param */
typedef enum {
    SNIFF_START_FOR_A2DP = 0x00, /* A2DP */
    SNIFF_START_FOR_HFP = 0x01,  /* HFP */
    SNIFF_START_FOR_HANDOVER = 0x02,
    SNIFF_START_FOR_INVALID
} SNIFF_START_OPERATION_TYPE;
typedef SNIFF_START_OPERATION_TYPE SNIFF_START_OPERATION_TYPE_E;

/* sniff_stop param */
typedef enum { SNIFF_STOP_MOBILE_DISC = 0x00, SNIFF_STOP_TWS_DISC = 0x01 } SNIFF_STOP_MONITOR_TYPE;
typedef SNIFF_STOP_MONITOR_TYPE SNIFF_STOP_MONITOR_TYPE_E;

/* sniff_handover param */
typedef enum {
    HANDOVER_SINGLE_NO_CONNECT,
    HANDOVER_DUAL_NO_CONNECT,
    HANDOVER_DUAL_NO_MONITOR,
    HANDOVER_DUAL_IN_MONITOR
} SNIFF_HANDOVER_TYPE;

typedef SNIFF_HANDOVER_TYPE SNIFF_HANDOVER_TYPE_E;
