...
// static stack_ble_advertiser_interface* stack_advertiser_interface = NULL;

const gatt_advertise_interface_t* get_ble_advertise_instance()
{
    // bluetooth_adapter = get_stack_bluetooth_adapter();
    ...
    // stack_advertiser_interface = bt_gatt->advertiser;
...
static void handle_event(void* data, size_t size)
{
    ...
    case ON_ADV_STOPPED: {
        advertise_hdl* handle = (advertise_hdl*)(msg->handle);
        // BT_CBACK(handle->callbacks, _ble_advertise_stopped_cb, handle->mgr_ctx, handle->advertiser_id);
        free(data);
        break;
    }
    default:
