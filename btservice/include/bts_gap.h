...
#define BTS_ADPT_XX(name, ret, ...) ret bts_adapt_##name(__VA_ARGS__)

#define BTS_ADPT_X(name, ...) BTS_ADPT_XX(name, bt_result_code, ##__VA_ARGS__)
...
typedef void (*bts_link_mode_change_callback)(bt_address remote_addr, bool active, uint16_t interval);
typedef void (*bts_scan_mode_change_callback)(bt_scan_mode scan_mode);
typedef void (*bts_link_encryption_change_callback)(bt_address remote_addr, bool br_link, bool encryption_on);
...
typedef struct {
    ...
    bts_received_remote_name_callback received_remote_name_cb;
    bts_discovery_state_changed_callback discovery_state_changed_cb;
    bts_ssp_request_callback ssp_request_cb;
    ...
    bts_link_mode_change_callback link_mode_change_cb;
    bts_scan_mode_change_callback scan_mode_change_cb;
    bts_link_encryption_change_callback link_encryption_change_cb;
...
BTS_ADPT_XX(bt_get_local_device_class, uint32_t, void);
BTS_ADPT_X(bt_get_remote_name, bt_device_t* device);
BTS_ADPT_XX(bt_get_remote_services, int, bt_device_t* remote_addr, bt_uuid_t* service_list, uint8_t count_in);
...
BTS_ADPT_X(bt_set_scan_mode, bt_scan_mode scanMode, bool bondable);
BTS_ADPT_X(bt_start_discovery, uint32_t timeout);
BTS_ADPT_X(bt_stop_discovery, void);
...
BTS_ADPT_X(bt_send_hci_command, bt_hci_command_t* command, hci_command_complete_event event_type);
BTS_ADPT_X(ble_set_static_identity, bt_device_t* device);
BTS_ADPT_X(ble_set_public_identity, bt_device_t* device);
BTS_ADPT_X(ble_get_current_irk, void);
BTS_ADPT_X(ble_set_address, bt_device_t* device);
BTS_ADPT_X(ble_get_address, void);
...
BTS_ADPT_X(ble_add_private_channel, uint16_t cid);
BTS_ADPT_X(ble_send_packet, bt_device_t* device, uint16_t private_cid, uint8_t* packet, uint16_t packet_size);
