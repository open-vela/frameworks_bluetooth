...
bt_service_callbacks service_callback = {
...
bt_result_code bts_if_init(void *handle, bt_service_if_callbacks* callbacks)
{
    if (!service) {
...
bt_result_code bts_if_enable(void* handle)
{
  bt_if_handle_t * if_handle;
    if(!handle)
...
bt_result_code bts_if_disable(void* handle)
{
    bt_if_handle_t * if_handle;
    if(!handle)
      return BT_RESULT_FAILED;
...
void bts_if_cleanup(void* handle)
{
    bt_if_handle_t * if_handle;
    if(!handle)
      return BT_RESULT_FAILED;
...
bt_manager_bt_state if_get_state(void * handle)
{
  if (!service)
...
bt_manager_ble_state if_get_ble_state(void * handle)
{
    if (!service)
      return BT_MANAGER_STATE_OFF;
    if(service->bt_state){
...
void bts_if_stack_state_change(stack_state_t state)
{
