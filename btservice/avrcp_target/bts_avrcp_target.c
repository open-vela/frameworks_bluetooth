...
static void bts_avrcp_target_received_get_play_status_request_callback(BD_ADDR remote_addr)
{
    BT_LOGD("%s", __func__);
}

static void bts_avrcp_target_received_get_element_attr_request_callback(BD_ADDR remote_addr)
{
    BT_LOGD("%s", __func__);
}

static void bts_avrcp_target_received_set_volume_callback(BD_ADDR remote_addr, uint8_t volume)
{
    BT_LOGD("%s, volume is %d", __func__, volume);
    //TODO add set volume to system through uinput
}

static void bts_avrcp_target_received_panel_operation_callback(BD_ADDR remote_addr,
