...
/* Verbose probe logging — compiled out unless CONFIG_BT_TRACE_SPP_LOG=y. */
#ifdef CONFIG_BT_TRACE_SPP_LOG
#  define SPP_TRACE_LOG(fmt, ...) BT_LOGD(fmt, ##__VA_ARGS__)
#else
#  define SPP_TRACE_LOG(fmt, ...)
#endif

/* Early-exit macro: check enabled + event filter in one shot */
#define TRACE_FILTER_OR_RETURN(event_id) \
    do { \
        if (!bt_trace_enabled()) return; \
        if (!spp_filter_event(event_id)) return; \
    } while (0)
...
/*
 * Pure ACL filter — checks all conditions without writing.
 * Returns true if the packet should be recorded.
 */
static inline bool hci_filter_acl(const uint8_t *hdr, uint16_t hdr_len)
{
    if (hdr_len < 2)
        return false;
    ...
    uint8_t dlci = hdr[8] >> 2;
    if (dlci == 0)
        return false;
    if (g_filter.dlci && dlci != g_filter.dlci)
        return false;

    uint8_t control = hdr[9];
    ...
    if (control & 0x10) {
        uint8_t len_bytes = (hdr[10] & 0x01) ? 1 : 2;
                                        ...
                                        uint16_t total_len, uint8_t acl_quota)
{
    ...
    if (hdr_len < 2)
        return false;
    if (((hdr[1] >> 4) & 0x03) == 0x01) {
        SPP_TRACE_LOG("[bt_trace] acl_record skip: continuation fragment");
        return false;
    }
    if (hdr_len < 11) {
        SPP_TRACE_LOG("[bt_trace] acl_record skip: hdr_len=%u < 11", hdr_len);
        return false;
    }

    uint16_t cid = (uint16_t)hdr[7] << 8 | hdr[6];
    if (cid < 0x0040) {
        SPP_TRACE_LOG("[bt_trace] acl_record skip: cid=0x%04x < 0x0040", cid);
        return false;
    }

    dlci = hdr[8] >> 2;
    if (dlci == 0) {
        SPP_TRACE_LOG("[bt_trace] acl_record skip: dlci=0 (mux ctrl)");
        return false;
    }
    if (g_filter.dlci && dlci != g_filter.dlci) {
        SPP_TRACE_LOG("[bt_trace] acl_record skip: dlci=%u != filter=%u",
                      dlci, g_filter.dlci);
        return false;
    }
    ...
    if ((control & 0xEF) != 0xEF) {
        SPP_TRACE_LOG("[bt_trace] acl_record skip: not UIH control=0x%02x",
                      control);
        return false;
    }
    ...
    if (hdr[10] & 0x01) {
        ...
        len_bytes = 2;
        if (hdr_len < 12) {
            SPP_TRACE_LOG("[bt_trace] acl_record skip: 2-byte len but hdr_len=%u",
                          hdr_len);
            return false;
        }
    ...
    if (control & 0x10) {
        ...
        if (credit_off < hdr_len)
            ...
            SPP_TRACE_LOG("[bt_trace] acl_record skip: truncated credits");
            return false;
        }
    }
    ...
    bt_trace_write_var(event_id, payload, sizeof(payload));
    SPP_TRACE_LOG("[bt_trace] acl_record: evt=0x%04x handle=0x%03x dlci=%u",
                  event_id, acl_handle, dlci);
    return true;
}
...
void bt_probe_hci_h4_rx_irq(uint8_t pkt_type, const uint8_t *buf, uint16_t len)
{
    ...
    if (pkt_type == 0x02) {
        if (!spp_filter_hci_conn(0x02, buf, len))
            return;
        hci_write_acl_record(BT_EVT_HCI_H4_RX_ACL, buf, len, len + 1, 0);
    } else if (pkt_type == 0x04) {
        if (len < 1 || buf[0] != 0x13)
            return;
        if (!spp_filter_hci_conn(0x04, buf, len))
            return;
        hci_write_evt_record(BT_EVT_HCI_H4_RX_EVT, buf, len, len + 1);
    }
}
...
void bt_probe_hci_h4_tx_done(const uint8_t *buf, uint16_t len)
{
    ...
    hci_write_acl_record(BT_EVT_HCI_H4_TX_DONE,
                         buf + 1, len - 1, len, 0);
    SPP_TRACE_LOG("[bt_trace] HCI_H4_TX_DONE len=%u", len);
}
...
void bt_probe_hci_stack_tx_acl(const uint8_t *buf, uint16_t len, uint8_t acl_quota)
{
    ...
    SPP_TRACE_LOG("[bt_trace] HCI_STACK_TX_ACL len=%u quota=%u", len, acl_quota);
}
                        ...
                        uint8_t peer_credits)
{
    ...
    SPP_TRACE_LOG("[bt_trace] RFCOMM_TX dlci=%u len=%u", dlci, data_len);
}
                        ...
                        uint8_t local_credits, uint8_t peer_credits_given)
{
    ...
    SPP_TRACE_LOG("[bt_trace] RFCOMM_RX dlci=%u len=%u", dlci, len);
}
