...
static int set_bitpool_cmd(void* handle, int argc, char* argv[])
{
    ...
    if (bt_a2dp_source_set_bitpool(handle, (uint8_t)bitpool) != BT_STATUS_SUCCESS)
        return CMD_ERROR;

    PRINT("Set A2DP source SBC max bitpool to %d", bitpool);
    return CMD_OK;
}

static void a2dp_src_connection_state_cb(void* cookie, bt_address_t* addr, profile_connection_state_t state)
{
