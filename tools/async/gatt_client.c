...
static int throughtput_state = 0;
typedef struct {
    int32_t run_time;
...
static void usage(void)
{
    ...
    printf("Commands:\n");
    for (int i = 0; i < ARRAY_SIZE(g_gattc_async_tables); i++) {
        printf("\t%-8s\t%s\n", g_gattc_async_tables[i].cmd, g_gattc_async_tables[i].help);
    }
}
...
static void create_connect_cb(bt_instance_t* ins, bt_status_t status, gattc_handle_t* phandle, void* userdata)
{
    int conn_id = *(int*)userdata;
...
static void delete_connect_cb(bt_instance_t* ins, bt_status_t status, void* userdata)
{
    if (status != BT_STATUS_SUCCESS)
        PRINT("delete connection fail, conn_id: %d", *(int*)userdata);
    else
        PRINT("delete connection success, conn_id: %d", *(int*)userdata);

    free(userdata);
}

static void get_attribute_cb(bt_instance_t* ins, bt_status_t status, gatt_attr_desc_t* attr_desc, void* userdata)
{
    uint8_t* b_uuid;
    uint8_t is_end_handle;
    if (status != BT_STATUS_SUCCESS)
        return;
    ...
    b_uuid = attr_desc->uuid.val.u128;
    ...
    is_end_handle = *(uint8_t*)userdata;
    if (is_end_handle) {
        printf(">");
...
static void write_cb(bt_instance_t* ins, bt_status_t status, void* userdata)
{
    if (userdata == NULL)
        return;

    throughtput_info_t* data = (throughtput_info_t*)userdata;
    if (data->run_time <= 0) {
    ...
    if (status != BT_STATUS_SUCCESS && throughtput_state == BT_STATUS_SUCCESS) {
        throughtput_state = status;
        uint32_t bit_rate = data->write_length * data->write_count / data->run_time;
        PRINT("gattc write throughput test finish, Bit rate = %" PRIu32 " Byte/s, = %" PRIu32 " bit/s, time = %" PRId32 "s.",
            bit_rate, bit_rate << 3, data->run_time);
    }
...
static int connect_cmd(void* handle, int argc, char* argv[])
{
    ...
    int conn_id = strtol(argv[0], NULL, 10);
    ...
    if (argc >= 3) {
        addr_type = strtol(argv[2], NULL, 10);
    ...
    if (bt_gattc_connect_async(g_gattc_devies[conn_id].handle, &addr, addr_type, status_cb, NULL) != BT_STATUS_SUCCESS)
        return CMD_ERROR;
...
static int disconnect_cmd(void* handle, int argc, char* argv[])
{
    ...
    if (bt_gattc_disconnect_async(g_gattc_devies[conn_id].handle, status_cb, NULL) != BT_STATUS_SUCCESS)
        return CMD_ERROR;
...
static int discover_services_cmd(void* handle, int argc, char* argv[])
{
    ...
    if (argc >= 2) {
        uint16_t uuid_val = (uint16_t)strtoul(argv[1], NULL, 16);
        uuid = BT_UUID_DECLARE_16(uuid_val);
        uuid_ptr = &uuid;
    }

    if (bt_gattc_discover_service_async(g_gattc_devies[conn_id].handle, uuid_ptr, status_cb, NULL) != BT_STATUS_SUCCESS)
        return CMD_ERROR;
...
static int read_request_cmd(void* handle, int argc, char* argv[])
{
    ...
    uint16_t attr_handle = strtoul(argv[1], NULL, 16);

    if (bt_gattc_read_async(g_gattc_devies[conn_id].handle, attr_handle, status_cb, NULL) != BT_STATUS_SUCCESS)
        return CMD_ERROR;
...
static int write_cmd(void* handle, int argc, char* argv[])
{
    ...
    if (!strcmp(argv[2], "str")) {
        if (bt_gattc_write_without_response_async(g_gattc_devies[conn_id].handle, attr_handle,
                (uint8_t*)argv[3], strlen(argv[3]), NULL, NULL)
            != BT_STATUS_SUCCESS)
            return CMD_ERROR;
        ...
        for (i = 0; i < len; i++)
            value[i] = (uint8_t)(strtoul(argv[3 + i], NULL, 16) & 0xFF);
        if (bt_gattc_write_without_response_async(g_gattc_devies[conn_id].handle, attr_handle,
                value, len, NULL, NULL)
            != BT_STATUS_SUCCESS)
            goto error;
...
static int write_request_cmd(void* handle, int argc, char* argv[])
{
    ...
    uint16_t attr_handle = (uint16_t)strtoul(argv[1], NULL, 16);

    if (!strcmp(argv[2], "str")) {
        if (bt_gattc_write_async(g_gattc_devies[conn_id].handle, attr_handle,
                (uint8_t*)argv[3], strlen(argv[3]), NULL, NULL)
            != BT_STATUS_SUCCESS)
            return CMD_ERROR;
...
static int enable_cccd_cmd(void* handle, int argc, char* argv[])
{
    ...
    uint16_t attr_handle = strtoul(argv[1], NULL, 16);
    uint16_t ccc_value = strtoul(argv[2], NULL, 16);

    if (bt_gattc_subscribe_async(g_gattc_devies[conn_id].handle, attr_handle, ccc_value, status_cb, NULL) != BT_STATUS_SUCCESS)
        return CMD_ERROR;
...
static int disable_cccd_cmd(void* handle, int argc, char* argv[])
{
    ...
    if (bt_gattc_unsubscribe_async(g_gattc_devies[conn_id].handle, attr_handle, status_cb, NULL) != BT_STATUS_SUCCESS)
        return CMD_ERROR;
...
static int exchange_mtu_cmd(void* handle, int argc, char* argv[])
{
    ...
    uint32_t mtu = strtoul(argv[1], NULL, 10);

    if (bt_gattc_exchange_mtu_async(g_gattc_devies[conn_id].handle, mtu, status_cb, NULL) != BT_STATUS_SUCCESS)
        return CMD_ERROR;
...
static int update_conn_cmd(void* handle, int argc, char* argv[])
{
    ...
    uint32_t min_interval = strtoul(argv[1], NULL, 0);
    uint32_t max_interval = strtoul(argv[2], NULL, 0);
    uint32_t latency = strtoul(argv[3], NULL, 0);
    uint32_t timeout = strtoul(argv[4], NULL, 0);
    uint32_t min_connection_event_length = strtoul(argv[5], NULL, 0);
    uint32_t max_connection_event_length = strtoul(argv[6], NULL, 0);

    if (bt_gattc_update_connection_parameter_async(g_gattc_devies[conn_id].handle, min_interval, max_interval, latency,
            timeout, min_connection_event_length, max_connection_event_length, status_cb, NULL)
        != BT_STATUS_SUCCESS)
        return CMD_ERROR;
...
static int read_phy_cmd(void* handle, int argc, char* argv[])
{
    ...
    if (bt_gattc_read_phy_async(g_gattc_devies[conn_id].handle, status_cb, NULL) != BT_STATUS_SUCCESS)
        return CMD_ERROR;
...
static int update_phy_cmd(void* handle, int argc, char* argv[])
{
    ...
    int tx = strtol(argv[1], NULL, 10);
    int rx = strtol(argv[2], NULL, 10);

    if (bt_gattc_update_phy_async(g_gattc_devies[conn_id].handle, tx, rx, status_cb, NULL) != BT_STATUS_SUCCESS)
        return CMD_ERROR;
...
static int read_rssi_cmd(void* handle, int argc, char* argv[])
{
    ...
    if (bt_gattc_read_rssi_async(g_gattc_devies[conn_id].handle, status_cb, NULL) != BT_STATUS_SUCCESS)
        return CMD_ERROR;
...
static int throughput_cmd(void* handle, int argc, char* argv[])
{
    ...
    int32_t test_time = strtol(argv[2], NULL, 10);
    if (test_time <= 0)
        return CMD_INVALID_OPT;
    ...
    while (1) {
        ...
        data = (throughtput_info_t*)malloc(sizeof(throughtput_info_t));
        data->run_time = run_time;
        data->write_count = write_count;
        data->write_length = write_length;

        int ret = bt_gattc_write_without_response_async(g_gattc_devies[conn_id].handle, attr_handle, payload, write_length, write_cb, (void*)data);
...
static void discover_callback(void* conn_handle, gatt_status_t status, bt_uuid_t* uuid, uint16_t start_handle, uint16_t end_handle)
{
    ...
    if (!uuid || !uuid->type) {
        uint8_t* is_end_handle = (uint8_t*)malloc(sizeof(uint8_t));
        *is_end_handle = true;

        ret = bt_gattc_get_attribute_by_handle_async(conn_handle, 0U, get_attribute_cb, (void*)is_end_handle);
        if (ret != BT_STATUS_SUCCESS)
            free(is_end_handle);
    ...
    for (uint16_t attr_handle = start_handle; attr_handle <= end_handle; attr_handle++) {
        if (bt_gattc_get_attribute_by_handle_async(conn_handle, attr_handle, get_attribute_cb,
                NULL)
            != BT_STATUS_SUCCESS)
            PRINT("gattc_get_attribute_by_handle fail, attr_handle: 0x%04x", start_handle);
    }
}
...
static int create_cmd(void* handle, int argc, char* argv[])
{
    ...
    for (index = 0; index < GATTC_CONNECTION_MAX; index++) {
        if (g_gattc_devies[index].handle == NULL)
            break;
    }

    if (index >= GATTC_CONNECTION_MAX) {
...
int gattc_command_uninit_async(void* handle)
{
    for (int i = 0; i < GATTC_CONNECTION_MAX; i++) {
        if (g_gattc_devies[i].handle) {
            bt_gattc_delete_connect_async(g_gattc_devies[i].handle, status_cb, NULL);
        }
    }
