...
static void on_acl_disconnected_cb(void* cookie, bt_address_t* addr, ble_addr_type_t addr_type, bt_transport_t transport, uint8_t reason)
{
    PRINT_ADDR("Device [%s][%s] disconnected, addr_type: %d, reason: 0x%02x", addr, LINK_TYPE(transport), addr_type, reason);
}
