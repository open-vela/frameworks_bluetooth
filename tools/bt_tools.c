...
static int discovery_cmd(void* handle, int argc, char** argv)
{
    ...
    if (!strcmp(argv[0], "start")) {
        ...
        int timeout = strtol(argv[1], NULL, 16);
...
static int set_scanmode_cmd(void* handle, int argc, char** argv)
{
    ...
    int scanmode = strtol(argv[0], NULL, 16);
...
static int set_iocap_cmd(void* handle, int argc, char** argv)
{
    ...
    int iocap = strtol(argv[0], NULL, 16);
...
static int set_le_iocap_cmd(void* handle, int argc, char** argv)
{
    ...
    uint32_t iocap = strtol(argv[0], NULL, 16);
    if (iocap < BT_IO_CAPABILITY_DISPLAYONLY || iocap > BT_IO_CAPABILITY_KEYBOARDDISPLAY)
        return CMD_INVALID_PARAM;
...
static int set_bondable_le_cmd(void* handle, int argc, char** argv)
{
    ...
    bool bondable = strtol(argv[0], NULL, 16);
...
static int set_security_level_cmd(void* handle, int argc, char** argv)
{
    ...
    int transport = strtol(argv[1], NULL, 16);
...
static int set_identity_addr_cmd(void* handle, int argc, char** argv)
{
    ...
    int type = strtol(argv[1], NULL, 16);
    if (type != 0 && type != 1) {
        return CMD_INVALID_PARAM;
    }
...
static int set_scan_parameters_cmd(void* handle, int argc, char** argv)
{
    ...
    int is_page = strtol(argv[0], NULL, 16);
    ...
    int window = strtol(argv[3], NULL, 16);
...
static int set_debug_mode_cmd(void* handle, int argc, char** argv)
{
    ...
    operation = strtol(argv[1], NULL, 16);
...
static int pair_reply_cmd(void* handle, int argc, char** argv)
{
    ...
    int reply = strtol(argv[1], NULL, 16);
    if (reply != 0 && reply != 1)
        return CMD_INVALID_PARAM;
...
static int pair_set_passkey_cmd(void* handle, int argc, char** argv)
{
    ...
    int reply = strtol(argv[2], NULL, 16);
    if (reply != 0 && reply != 1)
        return CMD_INVALID_PARAM;
...
static int le_connect_cmd(void* handle, int argc, char** argv)
{
    ...
    while ((opt = getopt_long(argc, argv, "a:t:f:p:l:T:dh", le_conn_options,
        ...
        case 't': {
            int32_t type = strtol(optarg, NULL, 16);
            addrtype = type;
        } break;
        case 'd': {
        ...
        case 'f': {
            int32_t filter = strtol(optarg, NULL, 16);
        ...
        case 'p': {
            int32_t phy = strtol(optarg, NULL, 16);
            if (!phy_is_vaild(phy)) {
        ...
        case 'l': {
            int32_t latency = strtol(optarg, NULL, 16);
        ...
        case 'T': {
            int32_t timeout = strtol(optarg, NULL, 16);
        ...
        case 0: {
            ...
            int32_t val = strtol(optarg, NULL, 16);
...
static int set_phy_cmd(void* handle, int argc, char** argv)
{
    ...
    int tx_phy, rx_phy;
    tx_phy = strtol(argv[1], NULL, 16);
    rx_phy = strtol(argv[2], NULL, 16);
...
static int get_bonded_devices_cmd(void* handle, int argc, char** argv)
{
    ...
    int transport = strtol(argv[0], NULL, 16);
    if (transport != BT_TRANSPORT_BREDR && transport != BT_TRANSPORT_BLE)
        return CMD_INVALID_PARAM;
