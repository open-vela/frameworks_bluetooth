...
static int start_scan_cmd(void* handle, int argc, char* argv[])
{
    ...
    while ((opt = getopt_long(argc, argv, "t:p:m:l:f:", scan_options,
        ...
        case 'm': {
            ...
            if (scanmode == 0) {
                param.interval = SCAN_MODE_LOW_POWER_INTERVAL;
                param.window = SCAN_MODE_LOW_POWER_WINDOW;
            } else if (scanmode == 1) {
                param.interval = SCAN_MODE_BALANCED_INTERVAL;
                param.window = SCAN_MODE_BALANCED_WINDOW;
            } else if (scanmode == 2) {
                param.interval = SCAN_MODE_LOW_LATENCY_INTERVAL;
                param.window = SCAN_MODE_LOW_LATENCY_WINDOW;
            } else {
    ...
    vela_mible_scan_start(&param, NULL);
