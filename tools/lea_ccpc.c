...
static void *ccpc_callbacks = NULL;
...
static void usage(void)
{
    ...
    printf("Commands:\n");
    for (int i = 0; i < ARRAY_SIZE(g_lea_ccpc_tables); i++) {
        printf("\t%-8s\t%s\n", g_lea_ccpc_tables[i].cmd, g_lea_ccpc_tables[i].help);
    }
}
...
static int ccpc_read_bearer_provider_name(void *handle, int argc, char *argv[])
{
    ...
    if (bt_lea_ccpc_read_bearer_provider_name(handle, &addr) != BT_STATUS_SUCCESS)
        return CMD_ERROR;
...
static int ccpc_read_bearer_uci(void *handle, int argc, char *argv[])
{
    ...
    if (bt_lea_ccpc_read_bearer_uci(handle, &addr) != BT_STATUS_SUCCESS)
        return CMD_ERROR;
...
static int ccpc_read_bearer_technology(void *handle, int argc, char *argv[])
{
    ...
    if (bt_lea_ccpc_read_bearer_technology(handle, &addr) != BT_STATUS_SUCCESS)
        return CMD_ERROR;
...
static int ccpc_read_bearer_uri_schemes_supported_list(void *handle, int argc, char *argv[])
{
    ...
    if (bt_lea_ccpc_read_bearer_uri_schemes_supported_list(handle, &addr) != BT_STATUS_SUCCESS)
        return CMD_ERROR;
...
static int ccpc_read_bearer_signal_strength(void *handle, int argc, char *argv[])
{
    ...
    if (bt_lea_ccpc_read_bearer_signal_strength(handle, &addr) != BT_STATUS_SUCCESS)
        return CMD_ERROR;
...
static int ccpc_read_bearer_signal_strength_report_interval(void *handle, int argc, char *argv[])
{
    ...
    if (bt_lea_ccpc_read_bearer_signal_strength_report_interval(handle, &addr) != BT_STATUS_SUCCESS)
        return CMD_ERROR;
...
static int ccpc_read_content_control_id(void *handle, int argc, char *argv[])
{
    ...
    if (bt_lea_ccpc_read_content_control_id(handle, &addr) != BT_STATUS_SUCCESS)
        return CMD_ERROR;
...
static int ccpc_read_status_flags(void *handle, int argc, char *argv[])
{
    ...
    if (bt_lea_ccpc_read_status_flags(handle, &addr) != BT_STATUS_SUCCESS)
        return CMD_ERROR;
...
static int ccpc_read_call_control_optional_opcodes(void *handle, int argc, char *argv[])
{
    ...
    if (bt_lea_ccpc_read_call_control_optional_opcodes(handle, &addr) != BT_STATUS_SUCCESS)
        return CMD_ERROR;
...
static int ccpc_read_incoming_call(void *handle, int argc, char *argv[])
{
    ...
    if (bt_lea_ccpc_read_incoming_call(handle, &addr) != BT_STATUS_SUCCESS)
        return CMD_ERROR;
...
static int ccpc_read_incoming_call_target_bearer_uri(void *handle, int argc, char *argv[])
{
    ...
    if (bt_lea_ccpc_read_incoming_call_target_bearer_uri(handle, &addr) != BT_STATUS_SUCCESS)
        return CMD_ERROR;
...
static int ccpc_read_call_state(void *handle, int argc, char *argv[])
{
    ...
    if (bt_lea_ccpc_read_call_state(handle, &addr) != BT_STATUS_SUCCESS)
        return CMD_ERROR;
...
static int ccpc_read_bearer_list_current_calls(void *handle, int argc, char *argv[])
{
    ...
    if (bt_lea_ccpc_read_bearer_list_current_calls(handle, &addr) != BT_STATUS_SUCCESS)
        return CMD_ERROR;
...
static int ccpc_read_call_friendly_name(void *handle, int argc, char *argv[])
{
    ...
    if (bt_lea_ccpc_read_call_friendly_name(handle, &addr) != BT_STATUS_SUCCESS)
        return CMD_ERROR;
...
static int ccpc_call_control_by_index(void *handle, int argc, char *argv[])
{
    ...
    if (bt_lea_ccpc_call_control_by_index(handle, &addr, opcode) != BT_STATUS_SUCCESS)
        return CMD_ERROR;
...
static int ccpc_originate_call(void *handle, int argc, char *argv[])
{
    ...
    if (bt_lea_ccpc_originate_call(handle, &addr, uri) != BT_STATUS_SUCCESS)
        return CMD_ERROR;
...
static int ccpc_join_calls(void *handle, int argc, char *argv[])
{
    ...
    if (bt_lea_ccpc_join_calls(handle, &addr, number, call_indexes) != BT_STATUS_SUCCESS)
        return CMD_ERROR;
...
static const lea_ccpc_callbacks_t lea_ccpc_cbs = {
    sizeof(lea_ccpc_cbs),
    ccpc_test_callback,
};
...
{
    ccpc_callbacks = bt_lea_ccpc_register_callbacks(handle, &lea_ccpc_cbs);
...
int lea_ccpc_command_exec(void *handle, int argc, char *argv[])
{
    ...
    while ((opt = getopt_long(argc, argv, "h", lea_ccpc_options, NULL)) != -1) {
        switch (opt) {
        case 'h':
            usage();
            return CMD_OK;
    ...
    if (argc > 0)
        ret = execute_command_in_table(handle, g_lea_ccpc_tables, ARRAY_SIZE(g_lea_ccpc_tables), argc, argv);
